...
function db_sync_product(array $p): void {
    ...
    try {
        ...
        if (empty($sku)) return;
        $qty   = (int)($p['stock'] ?? 0);
        $alert = (int)($p['alert_qty'] ?? 10);
        $high  = (int)($p['high_stock_qty'] ?? 50);
        $status = ($qty <= 0) ? 'out_of_stock' : (($qty <= $alert) ? 'low_stock' : 'in_stock');
        $img   = !empty($p['image']) ? $p['image'] : null;

        $stmt = $pdo->prepare("
            INSERT INTO `products` (`sku`, `name`, `category_name`, `cost_price`, `wholesale_price`, `retail_price`, `image`, `stock_qty`, `alert_qty`, `high_stock_qty`, `status`)
            VALUES (:sku, :name, :cat, :cost, :ws, :ret, :img, :stock, :alert, :high, :status)
            ON DUPLICATE KEY UPDATE 
                ...
                `image` = VALUES(`image`), `stock_qty` = VALUES(`stock_qty`), `alert_qty` = VALUES(`alert_qty`),
                `high_stock_qty` = VALUES(`high_stock_qty`), `status` = VALUES(`status`)
        ");
        $stmt->execute([
            ':sku'    => $sku,
            ':name'   => $p['name'] ?? '',
...
function db_sync_client(array $c): void {
    ...
    try {
        ...
        $pin = trim($c['password'] ?? ($c['secret_pin'] ?? '1234'));
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (:shop_name, :owner_name, :phone, :city_area, :credit_limit, :current_balance, :secret_pin, :status)
            ON DUPLICATE KEY UPDATE
                ...
                `secret_pin` = VALUES(`secret_pin`), `status` = VALUES(`status`)
        ");
        $stmt->execute([
            ':shop_name'       => $c['name'] ?? '',
...
function db_delete_client(string $clientName, string $phone = '', int $dbId = 0): void {
    if (!is_db_connected()) return;
    try {
        $pdo = get_db();
        if ($dbId > 0) {
        ...
        if (!empty($phone)) {
            $stmt = $pdo->prepare("DELETE FROM `clients` WHERE `phone` = :phone");
...
/**
 * Helper to upload and store product image
 */
function handle_product_image_upload(?array $file, ?string $existingImage = null): ?string {
    ...
    if ($moved) {
        // Delete previous custom uploaded image if it exists
        if (!empty($existingImage) && strpos($existingImage, 'uploads/inventory/') === 0 && file_exists(__DIR__ . '/../' . $existingImage)) {
...
function calculate_inventory_metrics(array $products): array {
    ...
    foreach ($products as $p) {
        ...
        $highQty  = isset($p['high_stock_qty']) ? (int)$p['high_stock_qty'] : 50;

        if ($stock <= 0) {
            $outOfStockCount++;
        } elseif ($stock <= $alertQty) {
            $lowStockCount++;
        } elseif ($stock >= $highQty) {
            $highStockCount++;
        } else {
            $inStockCount++;
        }
 ...
 */
function add_product(array $input, ?array $file = null): array {
    ...
    if (empty($sku)) {
        ...
        $prefix = !empty($clean) ? strtoupper(substr($clean, 0, 4)) : 'ITEM';
        $sku = 'PRD-' . $prefix . '-' . rand(100, 999);
    }
    ...
    $alertQty       = max(1, (int)($input['alert_qty'] ?? 10));
    $highStockQty   = max($alertQty + 1, (int)($input['high_stock_qty'] ?? 50));

    // Handle Image Upload or direct URL
    $imagePath = '';
    if ($file !== null && isset($file['error']) && $file['error'] === UPLOAD_ERR_OK) {
        $imagePath = handle_product_image_upload($file, null) ?? '';
    } elseif (!empty($input['image'])) {
        $imagePath = trim($input['image']);
    }
 ...
 */
function edit_product(string $origSku, array $input, ?array $file = null): array {
    $data = get_store_metrics();
    ...
    $existingImg    = $data['products'][$origSku]['image'] ?? '';
    ...
    $alertQty       = max(1, (int)($input['alert_qty'] ?? ($data['products'][$origSku]['alert_qty'] ?? 10)));
    $highStockQty   = max($alertQty + 1, (int)($input['high_stock_qty'] ?? ($data['products'][$origSku]['high_stock_qty'] ?? 50)));

    // Handle Image Upload / replacement / removal
    $imagePath = $existingImg;
    if ($file !== null && isset($file['error']) && $file['error'] === UPLOAD_ERR_OK) {
        $imagePath = handle_product_image_upload($file, $existingImg) ?? $existingImg;
    } elseif (!empty($input['remove_image']) && $input['remove_image'] == '1') {
        if (!empty($existingImg) && strpos($existingImg, 'uploads/inventory/') === 0 && file_exists(__DIR__ . '/../' . $existingImg)) {
...
function delete_product(string $sku): array {
    ...
    $existingImg = $data['products'][$sku]['image'] ?? '';
    if (!empty($existingImg) && strpos($existingImg, 'uploads/inventory/') === 0 && file_exists(__DIR__ . '/../' . $existingImg)) {
        @unlink(__DIR__ . '/../' . $existingImg);
    }
...
function get_clients_list(): array {
    ...
    if (is_db_connected()) {
        try {
            ...
            if (!empty($rows)) {
                ...
                foreach ($rows as $r) {
                    ...
                    $clients[$cid] = [
                        ...
                        'client_code' => 'WC-' . $idx,
                        'name'        => $r['shop_name'],
                        'owner'       => !empty($r['owner_name']) ? $r['owner_name'] : $r['shop_name'],
                        'phone'       => $r['phone'],
                        'password'    => !empty($r['secret_pin']) ? $r['secret_pin'] : '1234',
                        'city'        => !empty($r['city_area']) ? $r['city_area'] : 'Local Market',
                        'type'        => 'wholesale',
                        'balance'     => (float)($r['current_balance'] ?? 0),
                        'limit'       => (float)($r['credit_limit'] ?? 500000),
            ...
            error_log("Get Clients DB Error: " . $e->getMessage());
        }
    }
...
function add_client(array $clientData): array {
    ...
    if (empty($phone)) {
        return ['success' => false, 'message' => 'Client Mobile / Phone number is required for portal login'];
    }

    // Check if phone number is already assigned to another client
    $cleanNewPhone = preg_replace('/[^0-9]/', '', $phone);
    if (!empty($cleanNewPhone)) {
        foreach ($clients as $existing) {
            $existingClean = preg_replace('/[^0-9]/', '', $existing['phone'] ?? '');
            if (!empty($existingClean) && $cleanNewPhone === $existingClean) {
                return ['success' => false, 'message' => "Yeh Mobile Number ({$phone}) pehle se registered hai! Har client ka number alehda hona zaroori hai."];
            }
        }
    }

    // Password entered by admin or default to 1234
    $password = trim($clientData['password'] ?? ($clientData['pin'] ?? ''));
    if (empty($password)) {
        $password = '1234';
    }

    // Generate unique client code like WC-1004
    $clientCount = count($clients) + 1;
    $clientCode  = 'WC-' . str_pad((string)(1000 + $clientCount), 4, '0', STR_PAD_LEFT);
    $id = 'cli_' . strtolower(preg_replace('/[^a-zA-Z0-9]/', '', substr($name, 0, 8))) . '_' . rand(100, 999);
    $balance = max(0.0, (float)($clientData['balance'] ?? 0));
    $limit   = max(0.0, (float)($clientData['limit'] ?? 500000));
...
function edit_client(string $clientId, array $clientData): array {
    ...
    $phone = trim($clientData['phone'] ?? $clients[$clientId]['phone']);
    if (empty($phone)) {
        return ['success' => false, 'message' => 'Mobile number cannot be empty'];
    }

    // Check duplicate phone for other clients
    $cleanNewPhone = preg_replace('/[^0-9]/', '', $phone);
    if (!empty($cleanNewPhone)) {
        foreach ($clients as $cid => $existing) {
            if ($cid !== $clientId) {
                $existingClean = preg_replace('/[^0-9]/', '', $existing['phone'] ?? '');
                if (!empty($existingClean) && $cleanNewPhone === $existingClean) {
                    return ['success' => false, 'message' => "Yeh Mobile Number ({$phone}) kisi doosray client ke pass pehle se register hai."];
                }
            }
        }
    }
    ...
    if (!empty($clientData['password'])) {
        $clients[$clientId]['password'] = trim($clientData['password']);
    }
...
function delete_client(string $clientId): array {
    ...
    if (isset($clients[$clientId])) {
        ...
        foreach ($clients as $k => $c) {
            if (($c['id'] ?? '') === $clientId || 
                (string)($c['db_id'] ?? '') === (string)$clientId || 
                ($c['name'] ?? '') === $clientId ||
                ($c['phone'] ?? '') === $clientId) {
                $targetKey = $k;
                break;
    ...
    if ($targetKey !== null) {
        $name  = $clients[$targetKey]['name'] ?? '';
        $phone = $clients[$targetKey]['phone'] ?? '';
        $dbId  = (int)($clients[$targetKey]['db_id'] ?? 0);
        unset($clients[$targetKey]);
    } else {
    ...
    file_put_contents(get_clients_file_path(), json_encode($clients, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
    db_delete_client($name, $phone, $dbId);
    ...
    if (file_exists($file)) {
        ...
        if (is_array($all)) {
            if ($targetKey !== null) unset($all[$targetKey]);
            unset($all[$clientId]);
